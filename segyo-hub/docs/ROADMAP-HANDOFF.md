...
- 로드맵: **#1** 아이콘 세트(lucide) · **#3** 프로필 메뉴+로그아웃 · **#4** 다크모드 · **#5** 게시글 신고(`/admin/reports`) · **#6** 욕설 필터(마스킹+`/admin/words`+우회 의심 관리자 알림) · **#7** 게시판 구분(전체/자유/질문 탭) · **#8** 친구추가(`/friends`, `/u/[id]`) · **#9** 1:1 DM(`/messages`, 실시간+읽음 표시)
...
10. **멘션(@)** — 글/댓글에서 `@닉네임` 멘션 + 멘션된 사용자에게 알림. 아직 코드 없음.
...
## 마이그레이션 현황 (라이브 적용됨)
`0001`~`0020`. 최근: `0016` 욕설 우회 의심 알림, `0017` friendships, `0018` dm, `0019` dm 읽음 표시, `0020` 공지 info_card.
